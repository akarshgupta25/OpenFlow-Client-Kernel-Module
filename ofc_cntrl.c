...
int OfcCpHandleMultipart (__u8 *pCntrlPkt)
{
    ...
    switch(ntohs(request->type))
    {
        ...
        case OFPMP_PORT_DESC:
        {
            ...
            {
                printk (KERN_INFO "Failed to Send the Multipart Port Desc Reply\n");
                return OFC_FAILURE;
            }
        }
...
int ofcCpHandleMultipartSwitchDesc (__u8 *pCntrlPkt)
{
    ...
    tlvLength = sizeof(tOfcMultipartDesc) + sizeof(tOfcCpMultipartHeader);
    pMultipartHeader = (tOfcCpMultipartHeader *) kmalloc (tlvLength, GFP_KERNEL);
    ...
    memset(pMultipartHeader, 0, tlvLength);
    pMultipartHeader->type = htons(OFPMP_DESC);

    pReplyDesc = (tOfcMultipartDesc *) ((__u8 *)pMultipartHeader + 
...
int ofcCpHandleMultipartFlowStats (__u8 *pCntrlPkt)
{
    ...
    // Match TLV.
    pRequestOXMTlv = (tOfcMatchOxmTlv *)((__u8 *)pFlowStatsReq + 
                      sizeof(tOfcMultipartFlowStats) + 
                      2*sizeof(__u16));
    ...
    else
    {
        ...
        {
            if (ntohs(pRequestOXMTlv->Class) == OFPXMC_OPENFLOW_BASIC)
            {
                switch(pRequestOXMTlv->field >> 1)
                {
                    case OFCXMT_OFB_IN_PORT:
                    {
                        memcpy(&(fourByteField), 
                               pRequestOXMTlv->aValue, 
                               pRequestOXMTlv->length);
                        matchFields->inPort = htonl(fourByteField);
                        fourByteField = 0;
                        break;
                    }
                    case OFCXMT_OFB_ETH_DST:
                    ...
                    case OFCXMT_OFB_ETH_SRC:
                    {
                        memcpy(&(matchFields->aSrcMacAddr), 
                               pRequestOXMTlv->aValue, 
                               pRequestOXMTlv->length);
                        break;
                    }
    ...
    tableId = (pFlowStatsReq->tableId == OFC_ALL_TABLES ? OFC_FIRST_TABLE_INDEX : pFlowStatsReq->tableId);
    while (tableId < OFC_MAX_FLOW_TABLES)
    {
        ...
        list_for_each (pList, &pFlowTable->flowEntryList)
        {
            ...
            // Not the entry being queried for if the cookies dont match.
             if (!(pFlowStatsReq->cookie.hi == 0 && pFlowStatsReq->cookie.lo == 0) &&
                 pFlowStatsReq->cookie.hi == pFlowEntry->cookie.hi &&
                 pFlowStatsReq->cookie.lo == pFlowEntry->cookie.lo)
             {
                 continue;
             }

            if (pFlowStatsReq->outPort != OFPP_ANY &&
                htonl(pFlowStatsReq->outPort) != pFlowEntry->outPort)
            {
                continue;
            }
                        ...
                        sizeof(tOfcMatchFields)) == 0)
            {
                ...
                if (pFlowStatsReply)
                {
                    ...
                    if ((OfcCpAddOpenFlowHdr((__u8 *)pMultipartHeader, 
                        ...
                        (OfcCpSendCntrlPktFromSock(replyPkt, 
                                                   ntohs(((tOfcOfHdr *)replyPkt)->length)) != OFC_SUCCESS))
                    {
                        printk(KERN_INFO "Unable to send the multipart Flow Stats Reply \n");
                        return OFC_FAILURE;
                    }
                    kfree(replyPkt);
                    memset(pMultipartHeader, 0, OFC_MTU_SIZE);
                ...
                pFlowStatsReply = (tOfcMultiPartFlowStatsReply *) ((__u8 *)pMultipartHeader + 
                                   sizeof(tOfcCpMultipartHeader));
                ...
                pFlowStatsReply->cookie.hi = pFlowEntry->cookie.hi;
                pFlowStatsReply->cookie.lo = pFlowEntry->cookie.lo;
...
{
    tOfcCpMultipartHeader     *pMultipartHeader     = NULL;
    tOfcMultiPartPortDescRes  *pMultipartPortDesc   = NULL;
    __u8                      *replyPkt             = NULL;
    ...
    if (pMultipartHeader == NULL)
    {
        printk(KERN_INFO "Failed to allocate memory for multipart port desc"
               " message \n");
        return OFC_FAILURE;
    }
    memset(pMultipartHeader, 0, OFC_MTU_SIZE);
    ...
    lengthForProccessing = sizeof(tOfcCpMultipartHeader);
    ...
    if ((OfcCpAddOpenFlowHdr((__u8 *)pMultipartHeader,
        ...
        (OfcCpSendCntrlPktFromSock(replyPkt, 
                                   ntohs(((tOfcOfHdr *)replyPkt)->length)) != OFC_SUCCESS))
    {
        printk(KERN_INFO "Unable to send the message out\n");
        return OFC_FAILURE;
    }
    kfree(pMultipartHeader);
    kfree(replyPkt);
